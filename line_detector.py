...
def line_detector(image_data, line_info):
    ...
    try:
        mov_avg_left = np.append(line_info.mov_avg_left, np.array([line_info.left_fit]), axis=0)
        mov_avg_right = np.append(line_info.mov_avg_right, np.array([line_info.right_fit]), axis=0)

    except:

        mov_avg_left = np.array([line_info.left_fit])
        mov_avg_right = np.array([line_info.right_fit])

    line_info.left_fit = np.array([np.mean(mov_avg_left[::-1][:, 0][0:MOV_AVG_LENGTH]),
                         np.mean(mov_avg_left[::-1][:, 1][0:MOV_AVG_LENGTH]),
                         np.mean(mov_avg_left[::-1][:, 2][0:MOV_AVG_LENGTH])])

    line_info.right_fit = np.array([np.mean(mov_avg_right[::-1][:, 0][0:MOV_AVG_LENGTH]),
    ...
    if line_info.mov_avg_left.shape[0] > 1000:
        line_info.mov_avg_left = mov_avg_left[0:MOV_AVG_LENGTH]
    if mov_avg_right.shape[0] > 1000:
        line_info.mov_avg_right = mov_avg_right[0:MOV_AVG_LENGTH]
